...
/**
 * @deprecated Class MD5 is deprecated
 */

public class MD5
{

    public byte[] calculate(byte abyte0[])
    {
        return calculate(abyte0, 0, abyte0.length);
    }

    public byte[] calculate(byte abyte0[], int i, int j)
    {
        md5Init();
        md5Update(abyte0, i, j);
        md5Final();
        return digest;
    }

    public byte[] calculate(String s)
    {
        return calculate(s.getBytes());
    }
    ...
    private void md5Update(byte abyte0[], int i, int j)
    {
        byte abyte1[] = new byte[64];
        int l = (int)(count[0] >>> 3) & 63;
        if((count[0] += j << 3) < (long)(j << 3))
            count[1]++;
        count[1] += j >>> 29;
        int i1 = 64 - l;
        int k;
        if(j >= i1)
        {
            md5Memcpy(buffer, abyte0, l, i, i1);
            md5Transform(buffer);
            for(k = i1; k + 63 < j; k += 64)
            {
                md5Memcpy(abyte1, abyte0, 0, i + k, 64);
                md5Transform(abyte1);
            }
        ...
        md5Memcpy(buffer, abyte0, l, i + k, j - k);
    }
    ...
    public static void main(String args[])
    {
        ...
        System.out.println((new StringBuilder()).append("MD5(\"").append(s).append("\"):").append(CodeUtil.Bytes2FormattedText(abyte0)).toString());
    }
